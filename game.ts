...
export async function play_round(state: GameState, player_bid: number, graph = buildPipeline()): Promise<RoundOutcome> {
...
// CLI runner remains available behind an env flag to avoid executing during imports (e.g., in Next.js API)
if (process.env.RUN_CLI === "1") {
    (async () => {
        ...
        while (true) {
            ...
            const result = await play_round(state, p_bid, graph);

            if (result.round) {
                print_reveal(result.round);
                console.log("\nAI reasons:");
                (result.aiReasons ?? []).forEach((r, i) => console.log(`${i + 1}. ${r}`));

                const winner = result.round.winner;
                if (winner == null) {
                    console.log(`\nResult: TIE!\n`);
                }
                else {
            ...
            if (result.status === "ended") {
                console.log(result.message ?? "Game ended.");
                break;
            }
        }
